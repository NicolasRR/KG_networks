...
def get_kl_loss(model, target_layers, inputs, roles, lambda_map, criterion, ignore_index=-100, assistant_token=32001):
    ...
    with torch.no_grad():
        target_probabilities = model(input_ids=inputs, labels=labels).logits[:,min_value:,...].detach()*(roles!=2).view(-1,1,1)
    ...
    input_probabilities = model(input_ids=inputs, labels=labels).logits[:,min_value:,...]
    ...
    del labels, inputs, roles

    loss = criterion(input_probabilities, target_probabilities, reduce="none", log_target=True)
    loss = (loss*coefficients)/(~mask).sum()  
...
def train(model, optimizer, scheduler, train_data_loader, test_dataloader, target_layers, lambda_map, wandb_run, val_every=1000, epochs=1):
    ...
    for epoch in range(epochs):
        for tokenized_data, roles in tqdm(train_data_loader):
